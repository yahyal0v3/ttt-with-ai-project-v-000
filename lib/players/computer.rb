      ...
      WIN_COMBINATIONS.each do |win_combination|
        if self.token == "X"
          ...
          opponents_almost_win = opponents_positions if opponents_positions.length == 2
            ...
            opponents_positions = win_combination.select {|position| board.position(position) == "X"} unless win_combination.all? {|position| board.position(position) == "X" || board.position(position) == "O"}
            opponents_almost_win = opponents_positions if opponents_positions.length == 2
        end
      end
